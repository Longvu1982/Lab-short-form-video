...
console.log(userTrace_3min.length)
...
// User select default
let alg = algSelect.options[algSelect.selectedIndex].value
let testingTime = timeSelect.options[algSelect.selectedIndex].value
let traceFile = networkSelect.options[algSelect.selectedIndex].value
var total = userTrace_1min.reduce((a,b) => a+b ,0)
console.log(alg, testingTime, traceFile)

// Get data from dropdown

function calculateBufferedTime(var1, var2, var3) {
    // append data for calculation
    var alg_select, time_select, trace_select
    var firstTimeBuffer
    switch(var1) {
        case 'Next-one':
            alg_select = 1
    ...
    switch(var2) {
        case '1min':
            time_select = userTrace_1min;
            break
        case '3mins':
            time_select = userTrace_3min;
            break
    }
    switch(var3) {
        case 'trace_0':
            trace_select = networkTrace[0]
            break
        case 'trace_1':
            trace_select = networkTrace[1]
            break
        case 'trace_2':
            trace_select = networkTrace[2]
            break
        case 'trace_3':
            trace_select = networkTrace[3]
            break
    }
    firstTimeBuffer = (1000/trace_select[0]).toFixed(2)   // the first segment that needs downloaded after opening the app
                                                            // we can ignore this since every ALG need this first time buffer
    var totalBufferTime = firstTimeBuffer

    var remain = 15000
    var index = 0;
    var buffer_time = 0
    var totalWacthTime = 0
    for(let i = 0; i < time_select.length; i++){
        
        var overDownloadedTime = 0;
        var overDownloadedCap = 0;
        var downloaded = 0;
        var time
        var remain
        var overCap_temp = 0;
        totalWacthTime += time_select[i]
        for(let j = index; j <= trace_select.length; j++){
            downloaded += trace_select[j]
            if ( remain == 0) {
                time = 0;
                index = j;
                break;
            ...
            if(downloaded > remain){
                time = (trace_select[j]-(downloaded-15000))/trace_select[j] + j - index             // time to download the remaining
                index = j 
        ...
        if(time_select[i] >= time){
            overDownloadedTime = time_select[i]-time
            var time_temp = (Math.floor(time)+1) - time
            overCap_temp = time_temp* trace_select[index]
            var new_index = Math.floor(overDownloadedTime - time_temp)
            for(let j = index; j < index + new_index ; j++){
                overCap_temp += trace_select[j]
                console.log("overCap_temp:" + overCap_temp)    
            }
            
            overCap_temp += ( overDownloadedTime - time_temp - new_index ) * trace_select[index + new_index ]
            overCap_temp = overCap_temp > 15000 ? 15000 : overCap_temp

            index = index + new_index 
            overDownloadedCap += overCap_temp 
            remain = 15000 - overDownloadedCap
            console.log("remain:" + remain)
            // console.log("over download cap:" + overDownloadedCap + "Kb")
            // console.log("overDownloadedTime: "+ overDownloadedTime)
        }
        else { 
            remain = 15000
            // buffer_time += (time - time_select[i]) 
            var remain_temp = Math.floor(totalWacthTime) + 1 - totalWacthTime
            var buffer_temp = (1 - (remain_temp)) * trace_select[index]
            buffer_time += buffer_temp > 0 ? 1000/trace_select[index] : (1 - buffer_temp)/trace_select[index+1] + remain_temp
            console.log(`buffer: ${time - time_select[i]}`)
        }
}
    console.log("buffer: " + buffer_time)
    calculateButton.onclick = () => {
        $('.buffer').innerHTML = `Buffered Time: <span>${buffer_time.toFixed(2)}s</span>`
    }
}

//Calculate the wasteTime base on testing time

function calculateWasteTime(var1, var2, var3){
    var time, traceFileChoose, algChoose
    switch(var1){
        case '1min':
            time = userTrace_1min;
            break;
    ...
    switch(var2){
        case 'trace_0':
            traceFileChoose = networkTrace[0]
            break
        case 'trace_1':
            traceFileChoose = networkTrace[1]
            break
        case 'trace_2':
            traceFileChoose = networkTrace[2]
            break
        case 'trace_3':
            traceFileChoose = networkTrace[3]
            break
    }
    switch(var3){
        case 'Next-one':
    ...
    var downloaded_Kb = 0
    var final_buffer = 0 
    // console.log("time = " + time.reduce((a,b) => a+b, 0))
    for(let i = 0; i < time.length; i++){
        var total = 0
        if(i != 0) {
            total += (1 - (time[i-1] - Math.floor(time[i-1])))*traceFileChoose[index_2]
        }
        // console.log("next total: " + total)
        if(i == 0) index_2 += Math.floor(time[i]) 
        else index_2 += Math.floor(time[i] - (1 - (time[i-1] - Math.floor(time[i-1]))))  + 1
        
        // console.log(index_1, index_2)
        
        for (let j = index_1; j < index_2; j++){
            total += traceFileChoose[j] 
            // console.log(total)
        }
        total += (time[i] - Math.floor(time[i]))*traceFileChoose[index_2]
        // console.log(total)
        index_1 = index_2+1
        var temp = total 
        if (algChoose) {
        // Open for the first time, max download of 2 videos ~ 30000k
        if ( i == 0 ) {
            max = 30000
            if (temp > 30000) temp = 30000
        }  

        // Next time the max download allow number of video - download
        else {
            max = 15000 + (i+1)*15000 - downloaded_Kb
        ...
        if (!algChoose) {
            ...
            if ( i == 0 ) {
                max = 45000
                if (temp > 45000) temp = 45000
            }  
    
            // Next time the max download allow number of video - download
            else {
                max = 30000 + (i+1)*15000 - downloaded_Kb
                if (temp > max) temp = max
                }
            }
        downloaded_Kb += temp
        // console.log("max: " + max/1000)
        console.log("temp: " + temp/1000)
        wasteTime += temp/1000
        if (temp/1000 < time[i]) final_buffer += time[i]-temp/1000
    }
    console.log("final_buffer" + final_buffer)
    wasteTime -= time.reduce((a,b) => a+b,0)
    if (wasteTime < 0) wasteTime = 0
    calculateButton.addEventListener('click', () => {
        $('.waste').innerHTML = `Waste Time: <span>${wasteTime.toFixed(2)}s </span>`
        }
    )
}

// Handle calculate waste time base on user select

calculateWasteTime(testingTime, traceFile, alg)
calculateBufferedTime(alg,testingTime,traceFile)
algSelect.onchange = ()=>{
    alg = algSelect.options[algSelect.selectedIndex].value
    calculateWasteTime(testingTime, traceFile)
    calculateBufferedTime(alg,testingTime,traceFile)

}
timeSelect.onchange = ()=> {
    testingTime = timeSelect.options[timeSelect.selectedIndex].value
    calculateWasteTime(testingTime, traceFile, alg)
    calculateBufferedTime(alg,testingTime,traceFile)

}
networkSelect.onchange = ()=> {
    traceFile = networkSelect.options[networkSelect.selectedIndex].value
    console.log(alg,testingTime, traceFile)
    calculateWasteTime(testingTime, traceFile, alg)
    calculateBufferedTime(alg,testingTime,traceFile)
